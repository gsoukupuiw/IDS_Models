...
# CNN Architecture with Adjusted Convolutional Layers
model = Sequential()
model.add(Conv1D(filters=64, kernel_size=4, activation='relu', input_shape=(X_train_scaled.shape[1], 1)))
model.add(MaxPooling1D(pool_size=2))
model.add(Conv1D(filters=128, kernel_size=3, activation='relu'))
model.add(MaxPooling1D(pool_size=2))
# Adjust the kernel size for the third Conv1D layer to fit the reduced feature map size
model.add(Conv1D(filters=256, kernel_size=2, activation='relu'))  # Reduced kernel size to 2
# Optionally, remove or adjust the pooling layer if needed
model.add(Flatten())
